...
const int inputPinLeft = 4;
const int inputPinStop = 3;
const int inputPinRight = 2;
...
void setup() {
  ...
  pinMode(inputPinLeft, INPUT_PULLUP);
  pinMode(inputPinStop, INPUT_PULLUP);
  pinMode(inputPinRight, INPUT_PULLUP);
...
void loop(){
  // 포텐쇼미터 값을 읽어옴
  int potentioMeter = 1000;
  
  // CW로 회전시 모터를 CW방향으로 회전시킨다.
  if(digitalRead(inputPinLeft) == HIGH){
    //모터의 속도를 계산한다.
  ...
  // CCW로 회전시 모터를 CCW방향으로 회전시킨다.
    if(digitalRead(inputPinRight) == HIGH){
    //모터의 속도를 계산한다.  	
  ...
  // 중간에 위치 했을 경우 정지시킨다.  
  if(digitalRead(inputPinStop) == HIGH){
